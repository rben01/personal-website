...
[NOTE]
====
For alignment to work consistently, it's imperative that you be using a link:https://en.wikipedia.org/wiki/Monospaced_font[monospace font] -- a font in which every character has the same width (as opposed to a link:https://en.wikipedia.org/wiki/Typeface#Proportion[variable-width font]).
...
And of course this would not be possible in whitespace-sensitive languages such as Python.
